...
class RdfStore extends QuadStore {
  ...
  import(source, opts) {
    ...
    this.importFromStream(source, opts, (err) => {
  ...
  remove(source, opts) {
    ...
    assert(_.isObject(opts), 'The "opts" argument is not an object.');
    const emitter = new events.EventEmitter();
    this.removeFromStream(source, opts, (err) => {
      if (err) emitter.emit('error', err)
      else emitter.emit('end');
    });
    return emitter;
  ...
  _delput(oldQuads, newQuads, opts, cb) {
    ...
    QuadStore.prototype._delput.call(this, oldQuads.map(quad => store._importQuad(quad)), newQuads.map(quad => store._importQuad(quad)), opts, cb);
  }
