...
class RdfStore extends QuadStore {
    ...
    async getApproximateSize(matchTerms, opts) {
        const importedTerms = serialization.importTerms(matchTerms, this.defaultGraph, true, false);
    ...
    async searchStream(patterns, filters, opts) {
        if (_.isNil(opts))
            opts = {};
        const importedPatterns = patterns.map(pattern => serialization.importTerms(pattern, this.defaultGraph, true, false));
        const importedFilters = filters.map((filter) => {
            return {
                type: filter.type,
                args: filter.args.map(arg => serialization.importTerm(arg, false, this.defaultGraph, true, false)),
            };
        });
        ...
        const iterator = results.iterator.map((binding) => {
            return serialization.exportTerms(binding, this.defaultGraph, this.dataFactory);
        });
    ...
    async _delput(oldQuads, newQuads, opts) {
        ...
        return QuadStore.prototype._delput.call(this, importedOldQuads, importedNewQuads, opts);
    }
    _isQuad(obj) {
    ...
    _importTerm(term, isGraph, rangeBoundaryAllowed = false) {
        return serialization.importTerm(term, isGraph, this.defaultGraph, rangeBoundaryAllowed);
    }
    _importQuad(quad) {
        return serialization.importQuad(quad, this.defaultGraph);
    }
    _createQuadDeserializerMapper() {
        return (quad) => {
            return serialization.exportQuad(quad, this.defaultGraph, this.dataFactory);
        };
    }
    _getTermValueComparator() {
        return (a, b) => {
            const aSerializedValue = a._serializedValue || serialization.importTerm(a, false, this.defaultGraph, true, false);
            const bSerializedValue = b._serializedValue || serialization.importTerm(b, false, this.defaultGraph, true, false);
            if (aSerializedValue < bSerializedValue)
                return -1;
            else if (aSerializedValue === bSerializedValue)
                return 0;
            else
                return 1;
        };
