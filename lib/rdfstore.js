...
class RdfStore extends QuadStore {
  ...
  match(subject, predicate, object, graph) {
    ...
    this.getStream(matchTerms)
      .then((_iterator) => { iterator.source = _iterator; })
  ...
  async getStream(matchTerms, opts) {
    ...
    const iterator = await QuadStore.prototype.getStream.call(this, importedMatchTerms, opts);
    return iterator.map(this._createQuadDeserializerMapper());
  }
  ...
  async sparql(query, opts) {
    ...
    return { type: results.type, variables: results.variables, items };
  }
