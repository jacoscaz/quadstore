...
class RdfStore extends QuadStore {
  ...
  put(quads, opts, cb) {
    ...
    return QuadStore.prototype.put.call(this, quads.map(quad => store._importQuad(quad)), opts, cb);
  }
  ...
  del(quads, opts, cb) {
    ...
    assert(_.isObject(quads), 'The "quads" argument is not an object or an array.');
    if (!Array.isArray(quads)) quads = [quads];
    return QuadStore.prototype.del.call(this, quads.map(quad => store._importQuad(quad)), opts, cb);
  }
