...
class QuadStore extends events.EventEmitter {
  ...
  constructor(pathOrLevel, opts) {
    ...
    assert(_.isString(pathOrLevel) || isLevel(pathOrLevel), 'The "path" argument is not a string nor a LevelUP instance.');
    assert(_.isObject(opts), 'The "opts" argument is not an object.');
    if (opts.db) assert(isAbstractLevelDownClass(opts.db), 'The "opts.db" argument is not a subclass of AbstractLevelDOWN.');
    this._db = isLevel(pathOrLevel)
      ? pathOrLevel
      : levelup(pathOrLevel, { db: opts.db, valueEncoding });
  ...
  importFromStream(source, opts, cb) {
    ...
    if (!_.isFunction(cb)) {
      return new Promise(_importFromStream);
    }
    _importFromStream(cb.bind(null, null), cb);
  }

  removeFromStream(source, opts, cb) {
    const store = this;
    if (_.isNil(opts)) opts = {};
    assert(isReadableStream(source), 'The "source" argument is not a readable stream.');
    assert(_.isObject(opts), 'The "opts" argument is not an object.');
    assert(_.isNil(cb) || _.isFunction(cb), 'The "cb" argument is not a function.');
    function _removeFromStream(resolve, reject) {
    ...
    if (!_.isFunction(cb)) {
      return new Promise(_removeFromStream);
    }
    _removeFromStream(cb.bind(null, null), cb);
  }
