...
const sparqlPatternToQuads = (pattern, binding = {}) => {
    ...
    switch (pattern.type) {
        case 'bgp':
            quads.push(...pattern.triples.map(triple => replaceBindingInPattern(triple, binding)));
            break;
        case 'graph':
            quads.push(...pattern.triples.map(triple => replaceBindingInPattern({ ...triple, graph: pattern.name }, binding)));
            break;
        default:
