...
class RdfStore extends QuadStore {
  ...
  constructor(path, opts) {
    ...
    const superOpts = _.extend(
      { defaultContextValue: '_DEFAULT_GRAPH_' },
      opts,
      { contextKey: 'graph' }
    );
  ...
  _initialize() {
    ...
    server.listen(store._httpPort, store._httpAddr, (err) => {
      ...
      store._fragmentsClient = new ldfClient.FragmentsClient(store._httpBaseUrl + '/ldf#dataset');
      store.emit('ready');
    });
  }
  ...
  sparql(query) {
    ...
    const fragmentsClient = this._fragmentsClient;
  ...
  _importQuad(quad) {
    const importedQuad = {
      ...
      graph: quad.graph ? this._importTerm(quad.graph) : undefined
    };
  ...
  _exportTerms(terms) {
    const exportedTerms = _.mapValues(terms, term => this._exportTerm(term));
