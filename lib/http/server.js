...
class HttpServer extends http.Server {
  ...
  constructor(rdfStore, opts) {
    ...
    const sparqlController = new SparqlController(rdfStore);
    ...
    router.all('/sparql', (req, res, next) => {
      sparqlController.handleRequest(req, res, next);
    });
  ...
  _GETLdf(req, res, next) {
    ...
    asynctools.toCallback(async () => {
      ...
      try {
        responseContentType = await HttpServer._negotiate(res);
      ...
      utils.createArrayStream(metadata.generateQuads(
        routeUrl, thisUrl,
        estimatedTotalCount, config.perPageCount,
