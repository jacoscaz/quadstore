...
class RdfStore extends QuadStore {
  ...
  constructor(path, opts) {
    ...
    super(path, _.extend({}, opts, {contextKey: 'graph', defaultContextValue: 'DEFAULT_GRAPH'}));
    const store = this;
    assert(_.isObject(opts.dataFactory), 'Missing "opts.dataFactory" property.');
    store.dataFactory = opts.dataFactory;
    store._httpServerPort = opts.httpServerPort || '8883';
    store._httpServerAddr = opts.httpServerAddr || '127.0.0.1';
    store._httpServerBaseUrl = opts.httpServerBaseUrl || 'http://127.0.0.1:8883';
    store._httpServer = new HttpServer(this, {baseUrl: store._httpServerBaseUrl});
    store._httpServer.listen(store._httpServerPort, store._httpServerAddr, (err) => {
      if (err) {
        store.emit('error', utils.wrapError(err, 'Cannot start listening. Could not initialize store.'));
        return;
      }
      store.emit('ready');
      store._ldfClient = new ldfClient.FragmentsClient(store._httpServerBaseUrl + '/ldf#dataset');
    });
  }
