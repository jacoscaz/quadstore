...
class QuadStore extends events.EventEmitter {
  ...
  getByIndexStream(name, opts) {
    ...
    assert(_.isObject(opts), 'The "opts" argument is not an object.');
    const levelOpts = {};
    if (opts.start) levelOpts.gte = name + this._separator + opts.start;
    if (opts.end) levelOpts.lte = name + this._separator + opts.end + this._boundary;
    if (opts.limit) levelOpts.limit = opts.limit;
