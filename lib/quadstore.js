...
class QuadStore extends events.EventEmitter {
  ...
  getByIndexStream(name, opts) {
    ...
    assert(_.isObject(opts), 'The "opts" argument is not an object.');
    const levelOpts = {};
    if (_.isString(opts.gte)) levelOpts.gte = name + this._separator + opts.gte + this._separator;
    if (_.isString(opts.lte)) levelOpts.lte = name + this._separator + opts.lte + this._boundary;
    if (_.isString(opts.gt)) levelOpts.gt = name + this._separator + opts.gt + this._separator;
    if (_.isString(opts.lt)) levelOpts.lt = name + this._separator + opts.lt + this._separator;
    if (_.isNumber(opts.limit)) levelOpts.limit = opts.limit;
    if (_.isBoolean(opts.reverse)) levelOpts.reverse = opts.reverse;
    return this._db.createValueStream(levelOpts);
  }
  ...
  _quadToBatch(quad, opts) {
    ...
    if (this._indexes.length > 0) {
      for (let i = 0, index; i < this._indexes.length; i += 1) {
