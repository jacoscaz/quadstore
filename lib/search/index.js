...
const getBindingsIterator = async (store, parsedPattern) => {
    ...
    return { type: types_1.TSResultType.BINDINGS, iterator, sorting, variables };
};
const nestedLoopJoin = async (store, prevResult, nextStage) => {
    ...
    const getInnerIterator = async (outerBinding) => {
        ...
        const { iterator } = await getBindingsIterator(store, {
            type: base_1.TSSearchStageType.BGP,
            optional: false,
    ...
    return {
        ...
        type: types_1.TSResultType.BINDINGS,
    };
};
...
const parseSearchStage = (stage) => {
    switch (stage.type) {
        case base_1.TSSearchStageType.BGP:
            return parseBgpSearchStage(stage);
        case base_1.TSSearchStageType.LT:
        case base_1.TSSearchStageType.LTE:
        case base_1.TSSearchStageType.GT:
        case base_1.TSSearchStageType.GTE:
...
const applySearchStage = async (store, prevResult, nextStage) => {
    switch (nextStage.type) {
        case base_1.TSSearchStageType.BGP:
            if (prevResult.type !== types_1.TSResultType.BINDINGS) {
