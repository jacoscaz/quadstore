...
class RdfStore extends QuadStore {
  ...
  getApproximateSize(matchTerms, opts, cb) {
    ...
    return super.getApproximateSize(importedTerms, opts, cb);
  }
  ...
  searchStream(patterns, filters, opts) {
    ...
    return super.searchStream(importedPatterns, importedFilters, opts).map(
      binding => serialization.exportTerms(binding, this._defaultContextValue, this._dataFactory)
    );
  }

  sparql(query, opts, cb) {
    ...
    const bindings = [];
    const _sparql = (resolve, reject) => {
      this.sparqlStream(query, opts)
        .on('data', (binding) => { bindings.push(binding); })
        .on('end', () => { resolve(bindings); })
        .on('error', (err) => { reject(err); });
    };
    if (!_.isFunction(cb)) {
      return new Promise(_sparql);
    }
    _sparql(cb.bind(null, null), cb);
  }

  sparqlStream(query, opts, cb) {
    return sparql.sparqlStream(this, query, opts, cb);
  }
