...
function isReadableStream(stream) {
  return _.isObject(stream)
    && _.isFunction(stream.on)
    && _.isFunction(stream.read);
}
...
class RdfStore extends QuadStore {
  ...
  removeMatches(subject, predicate, object, graph) {
    // Check on arguments done in match()
   ...
   */
  deleteGraph(graph) {
    // Check on arguments done in removeMatches()
  ...
  put(quads, opts, cb) {
    ...
    if (_.isNil(opts)) opts = {};
    assert(_.isObject(quads), 'The "quads" argument is not an object or an array.');
  ...
  createReadStream(matchTerms, opts) {
    ...
    return QuadStore.prototype.createReadStream.call(this, importedMatchTerms, opts)
      .pipe(createExportStream(this));
  }

  query(matchTerms, opts) {
    if (_.isNil(matchTerms)) matchTerms = {};
    if (_.isNil(opts)) opts = {};
    assert(_.isObject(matchTerms), 'The "matchTerms" argument is not an object.');
    assert(_.isObject(opts), 'The "opts" argument is not an object.');
  ...
  _createOrderComparator(termNames) {
    ...
    return function orderComparator(quadA, quadB) {
      function _compare(quadA, quadB, termNames) {
        ...
        const termValueA = quadA[termName]._serializedValue;
        const termValueB = quadB[termName]._serializedValue;
