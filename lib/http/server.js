...
class HttpServer extends http.Server {
  ...
  constructor(rdfStore, opts) {
    ...
    router.use((req, res, next) => {
      ...
      res.on('finish', () => {
        const then = Date.now();
        debug(`${req.method} ${req.originalUrl} (${then - now}ms)`);
      });
      next();
    });
  ...
  _GETLdf(req, res, next) {
    ...
    asynctools.toCallback(async () => {
      ...
      debug(`offset: ${offset}`);
      debug(`limit: ${limit}`);
      debug(`quadCount: ${quadCount}`);
      debug(`approximateTotalCount: ${approximateTotalCount}`);
      debug(`estimatedTotalCount: ${estimatedTotalCount}`);
      debug(`routeUrl: ${routeUrl}`);
      debug(`thisUrl: ${thisUrl}`);
      debug(`nextPageUrl: ${nextPageUrl}`);
      debug(`prevPageUrl: ${prevPageUrl}`);
    }, next, false);
  }
